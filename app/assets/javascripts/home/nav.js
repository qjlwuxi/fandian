...
function ChooseCity(a){
    $(".body_top span").eq(0).html("").addClass("back").css("background", "url(../assets/newnavi/back.png) no-repeat");
...
$(function(){
    if ($.browser.msie) {
        if ($.browser.version == "6.0") {
			$(document.body).append('<div id="ie6_bg"></div><div id="ie6_tip"><div id="ie6_update">对不起，您的浏览器版本过低，您可以尝试：</div></div>');
            ...
            if ($.browser.version == "7.0") {
                $("#city,#area").css("margin-top", "6px");
                $(".body_top span").eq(0).css({
                    top: "2px",
                    position: "relative"
                })
            }
...
//    }, function(){
//        $(this).css({
//            background: "url(../assets/newnavi/backgroundshop.png) no-repeat"
//        })
//    });
    $(".cities li").find("span").bind("click", function(){
        ...
        $(".back").hover(function(){
            if ($(this).html() == "") {
                $(this).css("background", "url(../assets/newnavi/back-hover.png) no-repeat")
            }
        }, function(){
            if ($(this).html() == "") {
                $(this).css("background", "url(../assets/newnavi/back.png) no-repeat")
            }
        })
    });
...
//        }).find("ul").show().find("li").css({
//            border: "0px",
//            "margin-left": "10px"
//        }).addClass("shopareali").find("a").css("background", "url(../assets/newnavi/backgroundshop.png) no-repeat");
//        $(".shoparea ul").css("margin-left", "-11px");
//        $("#schools #" + $(this).attr("class")).show().siblings("ul").hide();
//        if ($targer.length > 0) {
...
//            $(".tip").remove();
//            $targer.eq(0).before(a);
//            $targer.eq($targer.length - 1).after(b);
//            $targer.eq($targer.length - 1).css({
//                border: "0px",
//                "margin-bottom": "-13px"
//            });
//            $(".tip").css("border", 0);
...
//        $("#schools").hide(10);
//        $(".shopareas").show();
//        $(".shopareas #" + $(this).attr("class")).show().siblings("ul").hide();
...
function back(){
    var a = $(".body_top span").filter(function(){
        return $(this).html() != ""
    }).length;
    switch (a) {
        case 1:
...
function FindPosition(){
    if ($("#findcity").val() != "") {
        ...
        e = "   <div class='Guess'></div><div class='Guess_detail'></div><div class='Guess_Info'><div id='Guess'><img src='assets/newnavi/loc.png' /><span>我们猜您在</span>" + c + "<div class='Error'>没猜对&nbsp;?<a id='showall'>显示全部>></a></div></div></div>";
        $("body").append(e);
        $(".Guess").width(b);
        $(".Guess").height(g);
...
function BackCity(){
    $(".body_top img").hide(10);
    $(".body_top span").html("").css("background", "").eq(0).html("请选择所在区域").removeClass("back");
